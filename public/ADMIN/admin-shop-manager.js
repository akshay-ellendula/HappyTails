    ...
    fetch('/admin/vendor-stats')
        ...
        .then(data => {
            if (data.success) {
                ...
                document.querySelector('.stats-container .stat-card:nth-child(2) .number').textContent = `₹${stats.totalRevenue ? stats.totalRevenue.toFixed(2) : '0.00'}`;
                document.querySelector('.stats-container .stat-card:nth-child(3) .number').textContent = stats.totalOrders || 0;
                ...
                document.getElementById('totalGrowthPercent').textContent = 
                    (stats.totalGrowthPercent >= 0 ? '+' : '') + stats.totalGrowthPercent.toFixed(0) + '% from last month';
                document.getElementById('revenueGrowthPercent').textContent = 
                    (stats.revenueGrowthPercent >= 0 ? '+' : '') + stats.revenueGrowthPercent.toFixed(0) + '% from previous month';
                document.getElementById('ordersGrowthPercent').textContent = 
                    (stats.ordersGrowthPercent >= 0 ? '+' : '') + stats.ordersGrowthPercent.toFixed(0) + '% from last month';
                document.getElementById('todaysOrdersChange').textContent = 
                    (stats.todaysOrdersChange >= 0 ? '+' : '') + stats.todaysOrdersChange.toFixed(0) + '% from yesterday';
            } else {
